...
$COLLECTED_DEVLIBRARY_RIGHT = $APPLICATION->GetGroupRight($module_id);
if ($COLLECTED_DEVLIBRARY_RIGHT>='R') :

IncludeModuleLangFile($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/main/options.php');
Loc::loadMessages(__FILE__);

$defaultValue = array('-' => GetMessage('COLLECTED.DEVLIBRARY.OPTIONS.UNDEFINED'));
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && strlen($RestoreDefaults)>0 && $COLLECTED_DEVLIBRARY_RIGHT=='W' && check_bitrix_sessid())
{
	$bWasUpdated = true;
...
$aTabs = array(
	array('DIV' => 'collected_sline_settings', 'TAB' => Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.SETTINGS'), 'ICON' => 'settings', 'TITLE' => Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.SETTINGS')),
);

$tabControl = new CAdminTabControl('tabControl', $aTabs);

$strWarning = '';
if ($_SERVER['REQUEST_METHOD'] == 'POST' && (strlen($Update) > 0 || strlen($Apply) > 0) && $COLLECTED_DEVLIBRARY_RIGHT == 'W' && check_bitrix_sessid())
{
    $bWasUpdated = true;
	...
	if ($bUseLocationRegion) {
		...
		EventManager::getInstance()->registerEventHandler(
		  ...
		  'OnPageStart',
		  $module_id,
		  '\Collected\DevLib\Sale\Location\Region',
		  'OnPageStart'
		);
...
?>
		...
		<td colspan="3">
			...
			foreach ($siteList as $val) {
				...
				<table cellspacing="5" cellpadding="0" border="0" width="100%" align="center">
					...
					<tr class="heading">
						<td colspan="2"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.LOCATION_TITLE')?></td>
					</tr>
					<tr>
						<td align="right" width="49%"><label for="DEFAULT_LOCATION_CODE-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.DEFAULT_LOCATION_CODE')?>:</label></td>
						<td width="51%">
							<?php
					...
					<tr>
						...
						<td align="right" width="49%"><label for="REPLACE_LOCATION_REGION-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.REPLACE_LOCATION_REGION')?>:</label></td>
					...
					<?php if (Loader::includeModule('sale')): ?>
						<tr class="heading">
							<td colspan="2"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.LOCATION_REGION')?></td>
						</tr>
						<tr>
							<?php $bUseLocationRegion = Option::get($module_id, 'use_location_region', 'Y', $val['ID']); ?>
							<td align="right" width="49%"><label for="USE_LOCATION_REGION-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.USE_LOCATION_REGION')?>:</label></td>
	...
	<?/*
							<tr>
								...
								$arLocationRegionType = array(
									'DOMAIN' => Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.LOCATION_REGION_TYPE.DOMAIN'),
									'SUBDOMAIN' => Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.LOCATION_REGION_TYPE.SUBDOMAIN'),
								);
								?>
								<td align="right" width="49%"><label for="LOCATION_REGION_TYPE-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.LOCATION_REGION_TYPE')?>:</label></td>
								<td width="51%">
	...
	*/?>
							<tr>
								...
								?>
								<td align="right" width="49%"><label for="LOCATION_REGION_IBLOCK_ID-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.LOCATION_REGION_IBLOCK_ID')?>:</label></td>
								<td width="51%">
					...
					<tr class="heading">
						<td colspan="2"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.FAKEPRICE_OPTIONS')?></td>
					</tr>
					<tr>
						<?php $bFakepriceActive = Option::get($module_id, 'fakeprice_active', 'Y', $val['ID']); ?>
						<td align="right" width="49%"><label for="FAKEPRICE_ACTIVE-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.FAKEPRICE_ACTIVE')?>:</label></td>
					...
					<?php if ($bFakepriceActive == 'Y'): ?>
						<tr>
							...
							<td align="right" width="49%"><label for="PROPCODE_CML2LINK-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.PROPCODE_CML2LINK')?>:</label></td>
						...
						<tr>
							...
							<td align="right" width="49%"><label for="PROPCODE_FAKEPRICE-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.PROPCODE_FAKEPRICE')?>:</label></td>
						...
						<tr>
							...
							<td align="right" width="49%"><label for="PRICE_FOR_FAKE-<?=$val['ID']?>"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.PRICE_FOR_FAKE')?>:</label></td>
							<td width="51%">
					...
					<tr>
						...
						?>
						<td align="right" width="49%"><?=Loc::getMessage('COLLECTED.DEVLIBRARY.OPTIONS.NO_PHOTO_PATH')?></td>
						<td width="51%"><input type="text" name="NO_PHOTO_PATH[<?=$val['ID']?>]" value="<?=$no_photo_path?>" />
							...
							<input type="button" value="<?=Loc::getMessage("COLLECTED.DEVLIBRARY.OPTIONS.BTN_FILEDIALOG")?>" OnClick="BtnFileDialogOpenNoPhoto()">
							<?php
							CAdminFileDialog::ShowScript(
								Array(
									"event" => "BtnFileDialogOpenNoPhoto",
									"arResultDest" => array("FORM_NAME" => "collected_devlibrary_option", "FORM_ELEMENT_NAME" => "no_photo_path[".$val['ID']."]"),
...
<?if($_REQUEST["back_url_settings"] <> ''):?>
	<input type="submit" name="Update" <?if ($COLLECTED_DEVLIBRARY_RIGHT<"W") echo "disabled" ?> value="<?=GetMessage("MAIN_SAVE")?>" title="<?=GetMessage("MAIN_OPT_SAVE_TITLE")?>">
<?endif?>
...
<?endif?>
	<input type="submit" name="RestoreDefaults" <?if ($COLLECTED_DEVLIBRARY_RIGHT<"W") echo "disabled" ?> title="<?echo GetMessage("MAIN_HINT_RESTORE_DEFAULTS")?>" onclick="return confirm('<?echo AddSlashes(GetMessage("MAIN_HINT_RESTORE_DEFAULTS_WARNING"))?>')" value="<?echo GetMessage("MAIN_RESTORE_DEFAULTS")?>">
	<?=bitrix_sessid_post();?>
<?$tabControl->End();?>
</form>
...
<?
echo BeginNote();
echo GetMessage('COLLECTED.DEVLIBRARY.OPTIONS.REPLACE_LOCATION_REGION.NOTE');
echo EndNote();
