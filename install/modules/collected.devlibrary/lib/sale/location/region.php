...
namespace Collected\DevLib\Sale\Location;
...
class Region
{
	...
	static function OnPageStart()
	{
		...
		if (is_array($arRegionCurrent))
		{
			$city = \Collected\DevLib\Sale\Location\Location::getMyCity();
			if ($city['ID'] !== $arRegionCurrent['LOCATION_ID'])
			{
				// self::set($arRegionCurrent);
				\Collected\DevLib\Sale\Location\Location::setMyCity($arRegionCurrent['LOCATION_ID']);
			}
		}
		else
		{
	...
	public static function set($arRegion = array())
	{
		...
		{
			foreach (GetModuleEvents('collected.devlibrary', 'OnSiteRegionSelected', true) as $arEvent)
			{
	...
	public static function getRegionIBlockID()
	{
		...
		$iRegionIBlockID = Option::get('collected.devlibrary', 'location_region_iblock_id', null, SITE_ID);
	...
	{
		if (Option::get('collected.devlibrary', 'use_location_region', 'Y', SITE_ID) !== 'Y')
		{
			return false;
		}
	...
	function OnGetOptimalPrice($intProductID, $quantity = 1, $arUserGroups = array(), $renewal = 'N', $priceList = array(), $siteID = false, $arDiscountCoupons = false)
	{
		...
		if ($arRegion)
		{
			return \Collected\DevLib\Catalog\Product::GetOptimalPrice($intProductID, $quantity, $arUserGroups, $renewal, $priceList, $siteID, $arDiscountCoupons, $arRegion);
		}
		else
		{
			return true;
