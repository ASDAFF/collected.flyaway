...
use \Collected\DevLib\Sale\Location\Location;
...
if (!Loader::includeModule('collected.devlibrary') || !Loader::includeModule('sale'))
{
...
if (empty($arResult['ERROR']) && check_bitrix_sessid())
{
    switch ($action)
	{
        case 'change':
			...
			$arRegions = \Collected\DevLib\Sale\Location\Region::getRegions();
			...
			if (is_array($arRegions) && count($arRegions) > 0)
			{
				...
				if (!$arRegionCurrent)
				{
					$arRegionCurrent = \Collected\DevLib\Sale\Location\Region::getDefaultRegion();
				}
