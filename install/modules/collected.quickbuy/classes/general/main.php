...
class CRSQUICKBUYMain
{
	public static function OnBeforePrologElementUpdate() {
		...
		if(CRSQUICKBUYTab::NeedAddTab()) {
			$VALUE_TYPE = 'F';
			$VALUE_TYPE = ($_REQUEST['collected_quickbuy_discount_type']=='P'?'P':$VALUE_TYPE);
			$VALUE_TYPE = ($_REQUEST['collected_quickbuy_discount_type']=='S'?'S':$VALUE_TYPE);
			$quickbuy_id = IntVal($_REQUEST['collected_quickbuy_id']);
			$arFields = array(
				'ELEMENT_ID' => IntVal($_REQUEST['collected_quickbuy_element_id']),
				'ACTIVE' => 'Y',
				'DATE_FROM' => $_REQUEST['collected_quickbuy_date_from'],
				'DATE_TO' => $_REQUEST['collected_quickbuy_date_to'],
				'DISCOUNT' => IntVal($_REQUEST['collected_quickbuy_discount']),
				'VALUE_TYPE' => $VALUE_TYPE,
				'CURRENCY' => htmlspecialchars($_REQUEST['collected_quickbuy_currency']),
				'AUTO_RENEWAL' => ($_REQUEST['collected_quickbuy_auto_renewal']=='Y'?'Y':'N'),
				'QUANTITY' => IntVal($_REQUEST['collected_quickbuy_quantity']),
			);
			
			if($quickbuy_id>0 && $_REQUEST['collected_quickbuy_active']=='Y') {
				CRSQUICKBUYElements::Update($quickbuy_id, $arFields);
			} elseif($quickbuy_id<1 && $_REQUEST['collected_quickbuy_active']=='Y') {
				CRSQUICKBUYElements::Add($arFields);
			} elseif($quickbuy_id>0 && $_REQUEST['collected_quickbuy_active']!='Y') {
