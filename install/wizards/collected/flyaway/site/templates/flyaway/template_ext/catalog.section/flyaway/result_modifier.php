...
if (!CModule::IncludeModule('collected.devlibrary')
	...
	|| !CModule::IncludeModule('collected.favorite')) {
	return;
}
...
if (is_array($arResult['ITEMS'])) {
	RSDevLib::GetDataForProductItem($arResult['ITEMS']);
	foreach ($arResult['ITEMS'] as $iItemKey => $arItem)
	{
		if (!empty($arItem['OFFERS']))
		{
			...
			{
				$arResult['ITEMS'][$iItemKey]['OFFERS_EXT'] = RSDevLibOffersExtension::GetSortedProperties($arItem['OFFERS'], $arParams['OFFER_TREE_PROPS'][$arItem['OFFERS'][$iOfferSel]['IBLOCK_ID']], array('OFFERS_SELECTED' => $iOfferSel));
			}
		}

		$arResult['ITEMS'][$iItemKey]['FIRST_PIC'] = RSDevLib::getElementPictures($arResult['ITEMS'][$iItemKey], $params, 1);
