...
if(CModule::IncludeModule('collected.digital') && CModule::IncludeModule('collected.devlibrary')){
	...
	RSDevLibOffersExtension::GetDataForProductItem($arResult['ITEMS'], $arData);
	if (!empty($arResult['OFFERS'])) {
		// Get sorted properties
		$arResult['OFFERS_EXT'] = RSDevLibOffersExtension::GetSortedProperties($arResult['OFFERS'],$arParams['OFFERS_PROPERTY_CODE']);
		// /Get sorted properties
    if (is_array($arResult['OFFERS']) && count($arResult['OFFERS']) > 0) {
        ...
        $arResult['OFFERS'][$keyss2]['IMAGES'] = RSDevLibOffersExtension::GetAllPictures(array(),$arOffer,$arAllPicParams);
        $arResult['OFFERS'][$keyss2]['IMAGES_SMALL'] = RSDevLibOffersExtension::GetAllPictures($arSizes,$arOffer,$arAllPicParams);
        // /Get all pictures [offers]
      }
    }
	}
...
$arResult['NO_PHOTO'] = RSDevLib::GetNoPhoto(array('MAX_WIDTH'=>$arParams['MAX_WIDTH'],'MAX_HEIGHT'=>$arParams['MAX_HEIGHT']));
