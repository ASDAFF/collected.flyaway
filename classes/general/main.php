...
class RsFlyaway
{
	public static function saveSettings() {
		...
		if(isset($_REQUEST['presets']) && in_array($_REQUEST['presets'],$presetsType)) {
			COption::SetOptionString('collected.flyaway', 'presets', htmlspecialchars($_REQUEST['presets']) );
		}

		$arFilterStyle = array('left', 'right');
		if(isset($_REQUEST['filterSide']) && in_array($_REQUEST['filterSide'],$arFilterStyle)) {
			COption::SetOptionString('collected.flyaway', 'filterSide', htmlspecialchars($_REQUEST['filterSide']) );
		}
		$bannerType = array('type1', 'type2', 'type3', 'type4', 'type5');
		if(isset($_REQUEST['bannerType']) && in_array($_REQUEST['bannerType'],$bannerType)) {
			COption::SetOptionString('collected.flyaway', 'bannerType', htmlspecialchars($_REQUEST['bannerType']) );
		}
		$openMenuType = array('type1', 'type2', 'type3');
		if(isset($_REQUEST['openMenuType']) && in_array($_REQUEST['openMenuType'],$openMenuType)) {
			COption::SetOptionString('collected.flyaway', 'openMenuType', htmlspecialchars($_REQUEST['openMenuType']) );
		}
		 
		$sidemenuType = array('light', 'dark');
		if(isset($_REQUEST['sidemenuType']) && in_array($_REQUEST['sidemenuType'], $sidemenuType)) {
			COption::SetOptionString('collected.flyaway', 'sidemenuType', htmlspecialchars($_REQUEST['sidemenuType']) );
		}
		
		if($_REQUEST['StickyHeader']=='Y') {
			COption::SetOptionString('collected.flyaway', 'StickyHeader', 'Y' );
		} else {
			COption::SetOptionString('collected.flyaway', 'StickyHeader', "N");
		}

		/*if($_REQUEST['blackMode']=='Y') {
			COption::SetOptionString('collected.flyaway', 'blackMode', 'Y' );
		} else {
			COption::SetOptionString('collected.flyaway', 'blackMode', "N");
		}*/
		

		if(isset($_REQUEST['gencolor']) && $_REQUEST['gencolor'] != "") {
			$gencolor = htmlspecialchars($_REQUEST['gencolor']);
			COption::SetOptionString('collected.flyaway', 'gencolor', $gencolor );
		} else {
			$gencolor = COption::GetOptionString('collected.flyaway', 'gencolor', 'ffe062' );
		}
		if(isset($_REQUEST['secondColor'])) {
			$secondColor = htmlspecialchars($_REQUEST['secondColor']);
			COption::SetOptionString('collected.flyaway', 'secondColor', $secondColor );
		} else {
			$secondColor = COption::GetOptionString('collected.flyaway', 'secondColor', '' );
		}
		...
		if(isset($_REQUEST['optionFrom'])) {
			if($_REQUEST['optionFrom']=='module') {
				COption::SetOptionString('collected.flyaway', 'optionFrom', 'module' );
			} elseif ($_REQUEST['optionFrom']=='session') {
				COption::SetOptionString('collected.flyaway', 'optionFrom', 'session' );
			}
		}
	...
	public static function getSettings($paramName='',$default='') {
		...
		$return = $default;
		if($paramName!='') {
			$optionFrom = COption::GetOptionString('collected.flyaway', 'optionFrom', 'module');
			if($optionFrom=='session') {
				if(isset($_SESSION[$paramName])) {
					$return = $_SESSION[$paramName];
				}
			} else {
				$return = COption::GetOptionString('collected.flyaway', $paramName, $default);
			}
		}
		return $return;
	}
	...
	public static function generateCssColorFile() {
		$color = COption::GetOptionString('collected.flyaway', 'gencolor', 'ffe062');
		$secondColor = COption::GetOptionString('collected.flyaway', 'secondColor', '555555');
		$reverse = COption::GetOptionString('collected.flyaway', 'blackMode', 'N');
	...
	function ShowPanel()
    {
        if ($GLOBALS["USER"]->IsAdmin() && COption::GetOptionString("main", "wizard_solution", "", SITE_ID) == "collected.flyaway")
        {
            $GLOBALS["APPLICATION"]->SetAdditionalCSS("/bitrix/wizards/collected/flyaway/css/panel.css"); 

            $arMenu = Array(
                Array(        
                    "ACTION" => "jsUtils.Redirect([], '".CUtil::JSEscape("/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardSiteID=".SITE_ID."&wizardName=collected:flyaway&".bitrix_sessid_get())."')",
            ...
            $GLOBALS["APPLICATION"]->AddPanelButton(array(
                "HREF" => "/bitrix/admin/wizard_install.php?lang=".LANGUAGE_ID."&wizardName=collected:flyaway&wizardSiteID=".SITE_ID."&".bitrix_sessid_get(),
                "ID" => "flyaway_wizard",
